...
class OrderService
{
    ...
    * @return Collection<int, Order>
    ...
    public function getOrders(array $filters, User $user): Collection
    {
        ...
        // Apply status filter (support single value or array)
        ...
        // Apply date filters (supports both exact match and range)
        if (!empty($filters['departure_date']) && !empty($filters['return_date'])) {
            // If both dates provided, filter by range (departure_date between the two dates)
            $query->whereBetween('departure_date', [$filters['departure_date'], $filters['return_date']]);
        } else {
            // If only one date provided, filter by exact match
